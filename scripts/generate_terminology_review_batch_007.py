#!/usr/bin/env python3
"""Generate Phase-2 terminology review Batch 007 evidence packets.

Selects the next 25 highest-priority unreviewed candidates with no exact
source-heading-token evidence. Frequency/structural breadth controls review
order only; this script records evidence and never writes review decisions.
"""
...
BATCH_NUM = "007"
BATCH_ID = f"NANNUL-TERM-REVIEW-{BATCH_NUM}"
EXPECTED_COUNT = 25
...
def main() -> None:
    candidates_obj = load_json(DATA / "grammatical-terminology-candidates.json")
    review_obj = load_json(DATA / "grammatical-terminology-review.json")
    nurpa_obj = load_json(DATA / "nurpa.json")
    tokens = load_ndjson(DATA / "token-occurrences.ndjson")
    ...
    if any(c["candidate_id"] in reviewed_ids for c in selected):
        raise SystemExit("Batch 007 selection includes an already-reviewed candidate")
    if any(c["evidence"]["source_heading_matches"] for c in selected):
        raise SystemExit("Batch 007 selection contains heading-supported candidate")
    ...
    for candidate in selected:
        ...
        if len(occurrence_record_ids) <= 12:
            ...
            for rids in unit_records.values():
                add_id(rids[0]); add_id(rids[-1])
            n = len(occurrence_record_ids)
            for i in range(12):
                add_id(occurrence_record_ids[round(i * (n - 1) / 11)])
            for rid in occurrence_record_ids:
        ...
        packet_candidates.append({
            ...
            "structural_units": [
                {"unit_id": uid, "unit_label_ta": record_by_id[rids[0]]["unit_label_ta"], "record_ids": rids}
    ...
    packet = {
        ...
        "batch_id": BATCH_ID,
        "selection_policy": "next 25 highest-priority unreviewed frequency-only candidates after Batches 001-006; no source-heading-token evidence",
        "decision_status": "context-packet-only-no-decisions",
        "candidate_count": len(packet_candidates),
        "candidates": packet_candidates,
    }
    OUT.mkdir(parents=True, exist_ok=True)
    (OUT / f"batch-{BATCH_NUM}-contexts.json").write_text(json.dumps(packet, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    summary = [
        "# Nannūl Terminology Contextual Review — Batch 007", "",
    ...
    for i, c in enumerate(packet_candidates, 1):
        summary += [
            f"## {i}. `{c['surface_form_ta']}` — `{c['candidate_id']}`", "",
    ...
    (OUT / f"batch-{BATCH_NUM}-contexts.md").write_text("\n".join(summary) + "\n", encoding="utf-8")

    for i, c in enumerate(packet_candidates, 1):
        lines = [
            f"# Nannūl Terminology Review Batch 007 — Candidate {i:02d}", "",
        ...
        for ctx in c["contexts"]:
            ...
            lines += [
                ...
                f"Record heading: {ctx['topic_heading_ta'] or '(none)'}", "", "```text", ctx["text_ta"], "```", "",
            ]
        (OUT / f"batch-{BATCH_NUM}-candidate-{i:02d}.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
