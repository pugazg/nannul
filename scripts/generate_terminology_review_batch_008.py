#!/usr/bin/env python3
"""Generate Phase-2 terminology review Batch 008 context packets.

Selection: the next 25 highest-priority unreviewed candidates with no exact
source-heading-token evidence after Batches 001–007. Frequency/structural breadth
...
BATCH_ID = "NANNUL-TERM-REVIEW-008"
EXPECTED_COUNT = 25
...
def main() -> None:
    ...
    if any(c["evidence"]["source_heading_matches"] for c in selected):
        raise SystemExit("Batch 008 selection unexpectedly contains heading-supported candidate")
    ...
    packet = {
        ...
        "batch_id": BATCH_ID,
        "selection_policy": "next 25 highest-priority unreviewed frequency-only candidates after Batches 001-007; no source-heading-token evidence",
        "decision_status": "context-packet-only-no-decisions",
        "candidate_count": len(packet_candidates),
        "candidates": packet_candidates,
    }
    OUT.mkdir(parents=True, exist_ok=True)
    (OUT / "batch-008-contexts.json").write_text(json.dumps(packet, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    summary = [
        "# Nannūl Terminology Contextual Review — Batch 008",
        "",
    ...
    (OUT / "batch-008-contexts.md").write_text("\n".join(summary) + "\n", encoding="utf-8")

    for index, c in enumerate(packet_candidates, start=1):
        lines = [
            f"# Nannūl Terminology Review Batch 008 — Candidate {index:02d}",
            "",
        ...
        (OUT / f"batch-008-candidate-{index:02d}.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
