#!/usr/bin/env python3
"""Generate Phase-2 terminology review Batch 004 context packets.

Selection: the next 25 highest-priority unreviewed candidates with no exact
source-heading-token evidence after Batches 001–003. Frequency/structural breadth
...
BATCH_ID = "NANNUL-TERM-REVIEW-004"
EXPECTED_COUNT = 25
...
def main() -> None:
    ...
    if any(c["evidence"]["source_heading_matches"] for c in selected):
        raise SystemExit("Batch 004 selection unexpectedly contains heading-supported candidate")
    ...
    packet = {
        ...
        "batch_id": BATCH_ID,
        "selection_policy": "next 25 highest-priority unreviewed frequency-only candidates after Batches 001-003; no source-heading-token evidence",
        "decision_status": "context-packet-only-no-decisions",
        "candidate_count": len(packet_candidates),
        "candidates": packet_candidates,
    }
    OUT.mkdir(parents=True, exist_ok=True)
    (OUT / "batch-004-contexts.json").write_text(
        json.dumps(packet, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )

    summary = [
        "# Nannūl Terminology Contextual Review — Batch 004",
        "",
    ...
    (OUT / "batch-004-contexts.md").write_text("\n".join(summary) + "\n", encoding="utf-8")

    for index, c in enumerate(packet_candidates, start=1):
        lines = [
            f"# Nannūl Terminology Review Batch 004 — Candidate {index:02d}",
            "",
        ...
        (OUT / f"batch-004-candidate-{index:02d}.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
