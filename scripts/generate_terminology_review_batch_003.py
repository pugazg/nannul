#!/usr/bin/env python3
"""Generate Phase-2 terminology review Batch 003 context packets.

Selection: the 25 highest-priority unreviewed candidates with no exact
source-heading-token evidence. Frequency/structural breadth determines review order
only; it is not treated as semantic confidence.
...
BATCH_ID = "NANNUL-TERM-REVIEW-003"
EXPECTED_COUNT = 25
...
def main() -> None:
    ...
    if any(c["evidence"]["source_heading_matches"] for c in selected):
        raise SystemExit("Batch 003 selection unexpectedly contains heading-supported candidate")
    ...
    for candidate in selected:
        ...
        # Review coverage: all records for modest-frequency forms; otherwise a broad
        # deterministic sample plus one record per structural unit and every variant-linked record.
        ...
        if len(occurrence_record_ids) <= 12:
            ...
            for rids in unit_records.values():
                add_id(rids[0])
                add_id(rids[-1])
            # Evenly distributed deterministic body sample, capped at 12 positions.
    ...
    packet = {
        ...
        "batch_id": BATCH_ID,
        "selection_policy": "25 highest-priority unreviewed frequency-only candidates; no source-heading-token evidence",
        "decision_status": "context-packet-only-no-decisions",
        "candidate_count": len(packet_candidates),
        "candidates": packet_candidates,
    }
    OUT.mkdir(parents=True, exist_ok=True)
    (OUT / "batch-003-contexts.json").write_text(
        json.dumps(packet, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )

    summary = [
        "# Nannūl Terminology Contextual Review — Batch 003",
        "",
    ...
    (OUT / "batch-003-contexts.md").write_text("\n".join(summary) + "\n", encoding="utf-8")

    for index, c in enumerate(packet_candidates, start=1):
        lines = [
            f"# Nannūl Terminology Review Batch 003 — Candidate {index:02d}",
            "",
        ...
        (OUT / f"batch-003-candidate-{index:02d}.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
